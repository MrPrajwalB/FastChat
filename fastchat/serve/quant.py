...
from torch.cpu.amp import custom_bwd, custom_fwd
import math
...
def autotune_warmup(model):
    ...
    for m in tqdm(range(0, 12)):
        ...
        for n, (k, qweight, scales, qzeros, g_idx, bits, maxq) in n_values.items():
            ...
            QuantLinearFunction.apply(a, qweight.cpu(), scales.cpu(), qzeros.cpu(), g_idx.cpu(), bits, maxq)

class QuantLinearFunction(torch.autograd.Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float16)
    def forward(ctx, input, qweight, scales, qzeros, g_idx, bits, maxq):
        output = torch.empty((input.shape[0], qweight.shape[1]), device='cpu', dtype=torch.float16)
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            grad_input = torch.empty((input_shape[0], input_shape[1]), device='cpu', dtype=torch.float32)
